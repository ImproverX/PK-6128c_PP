...
;
	.db  "Improver"
	RET		; для RST7
	.db  "TEST6128+08"	; метка версии прошивки
;
L_TROM:	MOV  A, M
	XRA  D
	MOV  D, A
...
;	OUT     00Dh	; Экран из банка 2
	LXI  SP,00000h	; начало
	XRA  A		; счётчик
L_TMCU:	PUSH D		; положить в стек DE, 64 раза
	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D		; 8
	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D		; 16
	LXI  D, 0A9A9h
	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D\	PUSH D		; 24
...
;	NOP\ NOP
;	OUT     00Ch	; палитра -- установка физического цвета, ещё раз -- для ПК-6128ц++ не требуется
...
;
	...
	MOV  B, A
	OUT  00Eh
	MOV  M, A	; <-33h в банк 2
	SUI  011h
	OUT  00Eh
	MOV  M, A	; <-22h в банк 3
	SUI  011h
	OUT  00Eh
	MOV  M, A	; <-11h в банк 0
	XRA  A
	OUT  00Eh
	MVI  M, 044h	; <-44h в банк 1
	MOV  A, B	; A=033h
	OUT  00Eh
	CMP  M 	
	JNZ	L_MTA	; <> 33h
...
L_SDV:	INR  C
	...
	JMP     L_KLP	; возврат
;
	.ORG (($ + 0FFh) & 0FF00h)	; палитра (должна быть выровнена по адресам с ХХ00h)
	.db 000h	; 0 - |        | чёрный		<-
	.db 052h	; 1 - | ■ ■  ■ | серый
...
;
;---------------------------------------------------------------------------------
; с адреса L_PRG1 дописываются "Тест техпрогона" и "Тест устройств" в бинарном виде
;---------------------------------------------------------------------------------
;
	.ORG 07FF8h
L_SWMEM:NOP
