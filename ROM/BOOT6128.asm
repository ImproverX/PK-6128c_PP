...
M_VAR1	.EQU    0DED0h	; DB/DW тут хранится указатель стека (КД) или начало буфера загрузки
...
L_0000:	DI
	...
	LXI  SP,0E000h	; стек на конец экрана
	JMP	L_NXT
;
...
L_NXT:	LXI  H, 0C13Eh
	...
	JZ	L_0039	; если нажата УС >>>
	MVI  B, 080h	; счётчик (вся память)
	LXI  SP,00000h	; стартовый адрес
L_0039:	LXI  H, 00000h	; чем заполнять
L_CLRM:	PUSH H
	DCR  C
	JNZ     L_CLRM
	DCR  B
	JNZ     L_CLRM	; цикл очистки памяти
	MOV  B, A	; B = 0, если была нажата УС
...
L_CLR2:	PUSH H
	...
	JMP	L_NXT3
;	
L_NXT2: LXI  D, 0D5ABh	; что 	-- метка загрузки с УС
	LXI  H, 0C2F2h	; куда
	SPHL
	PUSH D
	PUSH D
	PUSH D
	PUSH D
	LXI  D, 0FF00h	; завершение
	PUSH D
	MOV  M, D	; =FF
;
L_NXT3:	OUT     001h	; выставить в порт C ноль (вкл.мотор,погасить рус.)
...
L_00EF: CALL    L_NN	; << загрузка окончена
	IN      001h
	ANI     020h	; выделить нажатие клавиши СС
	MVI  A, 003h	; установка PC[1]=1 (выкл.реле)
	OUT     000h
	JZ	L_00F6	; если нажата СС, то не сбрасываем
...
L_0195:	MVI  B, 004h	; << проверка наличия сетевого адаптера
	...
	ORI     0E0h
	MOV  C, A
	MVI  A, 08Bh	; A - вывод, B,C - ввод, режим 0 (1000 1011)
	OUT     004h	; -> РУС ПУ
L_01A0:	MOV  A, C
	OUT     007h	; -> порт А ПУ
	XTHL
...
L_01E2:	IN      005h	; <- порт С ПУ
	...
	IN      006h	; <- порт В ПУ
	MOV  B, A
	MVI  A, 098h	; A,C[4-7] - ввод, B, C[0-3] - вывод, режим 0 (1001 1000)
	OUT     004h	; -> РУС ПУ
	MOV  A, C
	OUT     006h	; -> порт В ПУ ???
	XRA  A
...
L_0231:	CALL    L_02E6
	...
	ORA  A
	JNZ     L_0231
;	LXI  H, M_VAR1+021h	;M_VEF1
	LXI  H, M_VAR1+01Fh	;M_VEF1-2	; определяем последний блок
	MOV  A, M		;
	INX  H			;
	ADD  M			;
	DCR  A			;
...
L_02B7: IN      005h	; <- порт С ПУ
	...
	IN      006h	; <- порт В ПУ
	PUSH PSW
	MVI  A, 09Ah	; A,B,C[4-7] - ввод, C[0-3] - вывод, режим 0 (1001 1010)
	OUT     004h	; -> РУС ПУ
	XRA  A
	OUT     005h	; -> порт С ПУ
L_02CD: DCR  E
...
;
;==========================================================
L_04FA:	SUI     0CFh	; клавиша F2+F3?	; << проверка наличия КД
	JZ	L_04FZ
	MVI  A, 0FFh
L_04FZ:	INR  A		; A = 1 если нажато и A = 0 если нет.
...
L_SWMEM:MVI A, 003h
	...
	JMP	L_START
L_7FFF:	.db 000h	; контрольная сумма этой части ПЗУ
	.END
