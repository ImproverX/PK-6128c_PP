...
* [K155RE3n.bin](/K155PE3n.bin) -- прошивка для ПЗУ D10, оптимизированная для ПК-6128ц++, с постоянным округлением команд до кратности 4 тактам (рекомендуется). [K155RE3.bin](/K155PE3.bin) -- та же прошивка, но с кратностью "2 такта" торможения работы процессора при выводе бордюра и КСИ/ССИ, даёт небольшое ускорение, но возможны глюки. Оба варианта описаны в файле [K155PE3.txt](/K155PE3.txt).
...
* [HDD](/HDD/) -- внутренний контроллер накопителя на жёстких магнитных дисках (НЖМД, или HDD), программно совместим с МДОС и РДС. В качестве НЖМД могут использоваться различные SSD-IDE диски (например, DiskOnModule), а также переходники на CF и CD-карты. Также отдельно разведена плата с разъёмом для CF-карт вместо IDE.
...
Кварц Q4 (32,768 кГц) следует выбирать с характеристикой нагрузочной емкости 6 пФ, иначе точность часов будет неудовлетворительной.
...
* 22..4F -- (не используется)
* 50..58, 5F -- контроллер НЖМД (IDE HDD)
* 59..5E, 60..FF -- (не используется)
